    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
	  ...
	  if (s != "") {
		  ...
		  if (event == "telemetry") {
			  ...
			  double buffer = 5.0;  // min distance between cars in the same lane
			  ...
			  if (too_close) {
				  ...
				  for (int check_lane : successor_lanes) {
					  ...
					  double lane_cost;
					  if (abs(lane - check_lane))
						  lane_cost = 0.01;
					  else
						  lane_cost = 0.0;

					  //inefficiency cost
					  double inefficiency_cost = 0.2*sigmoid((ref_vel - lane_minspeed[check_lane]) / ref_vel);
					  ...
					  if (closest_dist < buffer) {
						  ...
						  buffer_cost = 0.5 * sigmoid(buffer / closest_dist);
					  }

					  double cost = lane_cost + inefficiency_cost + buffer_cost;
					  cout << "lane: " << check_lane << " buffer_cost: "<< buffer_cost << " lane_cost: " << lane_cost << buffer_cost << " inefficiency_cost: " << inefficiency_cost << endl;
					  if (cost < best_cost) {
				  ...
				  if (flag_CL || best_lane == lane)
					  ref_vel -= 0.224;	//mph in 0.02s, equal to 3 m/s2
			  ...
			  //if previous state is almost empty, use the car state as starting refefence.
			  if (prev_size < 2) {
			  ...
			  // The total number of points to output is 50
